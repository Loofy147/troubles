...
_log,_T,_OFF={},time.perf_counter_ns,os.getenv("BOLT_OFF")
def _fmt(ns):
...
def _emit(n,ns):
 if n not in _log:_log[n]=[1,ns,ns,ns];print(f"⚡ {n}  {_fmt(ns)}",file=sys.stderr)
 else:
  s=_log[n];s[0]+=1;s[1]+=ns
  if ns<s[2]:s[2]=ns
  if ns>s[3]:s[3]=ns
  m=s[0]
  if m in{2,5,10,50,100} or m%100==0:print(f"⚡ {n}  ×{m}  μ={_fmt(s[1]//m)}  [{_fmt(s[2])}…{_fmt(s[3])}]",file=sys.stderr)
class bolt:
 __slots__=('_l','_t')
 ...
 def deep(f,*a,top=8,**k):
  ...
  pr=cProfile.Profile();r=pr.runcall(f,*a,**k);s=io.StringIO();pstats.Stats(pr,stream=s).strip_dirs().sort_stats('cumtime').print_stats(top);print(s.getvalue(),file=sys.stderr);return r
 @staticmethod
 def stats(n=None):
  for k,s in({n:_log[n]}if n and n in _log else _log if not n else {}).items():print(f"{k:20s} n={s[0]:>5}  total={_fmt(s[1])}  μ={_fmt(s[1]//s[0])}  min={_fmt(s[2])}  max={_fmt(s[3])}")
 @staticmethod
